...
public class ClassUtils {
     ...
     * @return
     */
    public static Set<Class<?>> getAllSubClassByClassPacket(Class c) {
        Set<Class<?>> returnClassList = new HashSet();
        String packageName = c.getPackage().getName();  //获得当前包名
        try {
            Set<Class<?>> allClass = getClassSetByPackageName(packageName);//获得当前包以及子包下的所有类

            for (Class<?> target : allClass) {
                //判断是否是超类
                if (c.isAssignableFrom(target)) {
                    //不是接口 且不是 抽象类
                    if (!c.equals(target) && !Modifier.isAbstract(target.getModifiers())) {
                        returnClassList.add(target);
                    }
                }
            }
        } catch (Exception e) {
